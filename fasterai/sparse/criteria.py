...
class Criteria():
    ...
    def __call__(self, m, granularity):
        ...
        if granularity == 'weight':
            ...
            wf = self.f(m.weight).mean(dim=1, keepdim=True)
            if self.needs_init: wi = self.f(m._init_weights).mean(dim=1, keepdim=True)
            elif self.needs_update: wi = self.f(m._old_weights).mean(dim=1, keepdim=True)

        elif granularity == 'vector':
            wf = self.f(m.weight).mean(dim=2, keepdim=True)
            if self.needs_init: wi = self.f(m._init_weights).mean(dim=2, keepdim=True)
            elif self.needs_update: wi = self.f(m._old_weights).mean(dim=2, keepdim=True)

        elif granularity == 'row':
            wf = self.f(m.weight).mean(dim=3, keepdim=True)
            if self.needs_init: wi = self.f(m._init_weights).mean(dim=3, keepdim=True)
            elif self.needs_update: wi = self.f(m._old_weights).mean(dim=3, keepdim=True)

        elif granularity == 'kernel':
            wf = self.f(m.weight).mean(dim=(2,3), keepdim=True)
            if self.needs_init: wi = self.f(m._init_weights).mean(dim=(2,3), keepdim=True)
            elif self.needs_update: wi = self.f(m._old_weights).mean(dim=(2,3), keepdim=True)

        elif granularity == 'filter':
            wf = self.f(m.weight).mean(dim=(1,2,3), keepdim=True)
            if self.needs_init: wi = self.f(m._init_weights).mean(dim=(1,2,3), keepdim=True)
            elif self.needs_update: wi = self.f(m._old_weights).mean(dim=(1,2,3), keepdim=True)
